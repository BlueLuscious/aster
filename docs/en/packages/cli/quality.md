...
| Transitional snapshot contracts | Export and Review still reconstruct complete provider state before exact selection. | Remove after exact definition workflows no longer consume them. |
...
| Catalogue concentration | Discovery normalisation is split by identity, metadata, records, and membership; legacy snapshot acceptance remains transitional. | Preserve semantic boundaries and remove the legacy path after exact command migration rather than layering over it. |
