...
test("compiles and imports an adopted editable icon module", async () => {
  const temporaryRoot = await mkdtemp(
    join(tmpdir(), "aster-adopted-icon-"),
  );
  ...
  try {
    ...
    const generated = await readFile(iconPath, "utf8");
    ...
    const adopted = generated
      .slice(moduleStart)
      .replace(
        '"displayName": "frame",',
        '"displayName": "Adopted Frame",\n    "tags": [\n      "adopted"\n    ],',
      );

    assert.notEqual(adopted, generated.slice(moduleStart));
    assert.doesNotMatch(adopted, /@generated by @aster\/build/u);
    await writeFile(iconPath, adopted, "utf8");
    await installCoreDependency(packageRoot);
    compilePackage(resolve(packageRoot, "tsconfig.json"));
    await installGeneratedPackage(packageRoot, consumerRoot);
    await mkdir(consumerRoot, { recursive: true });
    ...
    const loader = await import(
      `${pathToFileURL(resolve(consumerRoot, "loader.mjs")).href}?adopted-icon`
    ) as {
      readonly load: (
        specifier: string,
      ) => Promise<Record<string, unknown>>;
    };
    const frameModule = await loader.load("@aster/fixture/frame");
    const frame = frameModule.Frame as IconDefinition;

    assert.equal(frame.metadata.displayName, "Adopted Frame");
    assert.deepEqual(frame.metadata.tags, ["adopted"]);
