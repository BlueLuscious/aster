...
/**
 * @description Accepts the exact structured export invocation family.
 */
export class ExportInvocationNormaliser implements ICommandInvocationNormaliser {
  /**
   * @description Command identity owned by this normaliser.
   */
  readonly command = asterCommandNames.export;
  ...
  /**
   * @description Export-option acceptance boundary.
   */
  readonly #options = new ExportOptionsNormaliser();
   ...
   * @param value - Candidate export invocation.
  ...
  normalise(value: unknown): TAcceptanceResult<AsterCommandInvocationType> {
    const record = this.#data.record(
      value,
      ["command", "subject", "identity", "catalogue", "options"],
      ["command", "subject", "identity"],
    );

    if (record === undefined || record.command !== this.command) {
      return this.#rejections.invalid("export invocation contains an unknown field");
    }

    if (
      record.subject !== asterCommandSubjects.export.icon
      && record.subject !== asterCommandSubjects.export.collection
    ) {
    ...
    const validIdentity = record.subject === asterCommandSubjects.export.icon
    ...
    const options = this.#options.normalise(
      record.options,
